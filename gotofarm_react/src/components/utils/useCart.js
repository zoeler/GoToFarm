...
export const CartProvider = ({
  ...
  let initCoupon = { sid: 0, quota: 0, cate: 0 }
  // try {
  //   initCoupon = JSON.parse(localStorage.getItem('usecoupon'))
  // } catch (ex) {}

  //建立優惠卷
  const [myCoupon, setMyCoupon] = useState(initCoupon)

  //建立付款狀態
  const [myPayment, setMyPayment] = useState('信用卡')
